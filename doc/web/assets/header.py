...
def make_menu(filepath):
    ...
    @param filepath: path to HTML file of webpage
    @type filepath: `str`
    @rtype: `str`
    """
...
def _make_relative_path_to_content_dir(filepath):
...
def _find_branch(index, level, path):
    ...
    @param index: current position in
        the list `MENUMAP`
    @param level: current level being filled
...
def _on(prefix, depth, index, path, branch):
...
def _colorize_tla(tla_text):
    ...
    for keyword in tla_keywords:
        tla_text = re.sub(
            keyword,
            f'<span class="purple">{keyword}</span>',
            tla_text)
    return tla_text
